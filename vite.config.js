...
const header = fs.readFileSync(
  resolve(__dirname, 'src/layouts/header.html'),
  'utf-8',
);
const footer = fs.readFileSync(
  resolve(__dirname, 'src/layouts/footer.html'),
  'utf-8',
);
const head = fs.readFileSync(
  resolve(__dirname, 'src/layouts/head.html'),
  'utf-8',
);
...
function layoutInjectPlugin() {
  return {
    ...
    transformIndexHtml(html) {
      ...
      const hasWhatsappFloatPlaceholder = html.includes(
        '<!--whatsapp-float-->',
      );
      console.log(
        '占位符存在状态：header=',
      ...
      if (stillHasHeader || stillHasFooter || stillHasWhatsappFloat) {
        ...
        if (
          ...
          result.includes('<!--whatsapp-float-->')
        ) {
          console.error(
            '强制替换仍然失败，请检查布局文件是否为空或包含非法字符',
          );
        } else {
          console.log('强制替换成功');
        }
...
export default defineConfig({
  ...
  css: {
    devSourcemap: true, // CSS 也生成 Source Map
  },
